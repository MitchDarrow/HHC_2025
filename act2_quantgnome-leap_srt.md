...
We then use the hint provided by <code>pqc-keygen -t</code> to identify the target user of our first quantgnome leap: <code>gnome1</code>. This is the result of listing detailed information about our <code>.ssh/id_rsa</code> file via <code>ssh-keygen -l -f</code>. 
We use this RSA key to make our first leap. The <code>.ssh</code> directory of the <code>gnome1</code> user contains an ED25519 key which we can use to authenticate as <code>gnome2</code> with the <code>ssh -i</code> command. We are told that both the RSA and ED25519 key generation algorithms are not safe in a post-quantum world due to the reduction of the time complexity requirements in solving classical algorithms. 
